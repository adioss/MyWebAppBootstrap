...
import {listPersonsSuccess, getPersonSuccess, createPerson} from '../../actions/person';
...
function getInitialState() {
    return Map({persons: Map(), person: Map()});
}
...
describe('Persons reducer',
         () => {
             ...
             it('should return the state with two persons', () => {
                    const testedValue = {
                        'persons': [
                            createMockedPerson(1, 'test'),
                            createMockedPerson(2, 'test:')
                        ]
                    };
